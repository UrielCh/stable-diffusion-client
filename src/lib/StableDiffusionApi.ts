...
export default class StableDiffusionApi {
  ...
  public setAuth(username: string, password: string): StableDiffusionApi {
    if (username && password) {
      this.basicAuth = "Basic" + encodeBase64(username + ":" + password);
    }
    return this;
  }
  ...
  public async txt2img(
  ...
  ): Promise<StableDiffusionResult<Txt2ImgOptions, Txt2ImgInfo>> {
    let alwayson_scripts = await createScriptsWithCnUnits(
    ...
    const response = await this.post<ApiRawResponse>("/sdapi/v1/txt2img", {
  ...
  public async img2img(
  ...
  ): Promise<StableDiffusionResult<Img2ImgOptions, unknown>> {
    ...
    const response = await this.post<ApiRawResponse>("/sdapi/v1/img2img", {
      init_images,
  ...
  ): Promise<StableDiffusionResult<ExtraSingleOptions, unknown>> {
    const image = await toBase64(options.image);
    const response = await this.post<ApiRawResponse>(
  ...
  ): Promise<StableDiffusionResult<ExtraBatchOptions, unknown>> {
   ...
   */
  public async pngInfo(image: Sharp): Promise<StableDiffusionResult<unknown, unknown>> {
    const image_data = await toBase64(image);
    const response = await this.post<ApiRawResponse>("/sdapi/v1/png-info", {
      image: image_data,
    });
    return new StableDiffusionResult(response);
  }
  ...
  public async interrogate(
    image: Sharp,
    model: string,
  ): Promise<StableDiffusionResult<unknown, unknown>> {
    const image_data = await toBase64(image);
    const response = await this.post<ApiRawResponse>(
  ...
  public async getSamplers(): Promise<Sampler[]> {
    const response = await this.get<Sampler[]>("/sdapi/v1/samplers");
    return response;
  }
  ...
  public async getUpscalers(): Promise<Upscaler[]> {
    const response = await this.get<Upscaler[]>("/sdapi/v1/upscalers");
    return response;
  }
  ...
  public async getSdModels(): Promise<StableDiffusionModel[]> {
    const response = await this.get<StableDiffusionModel[]>(
      "/sdapi/v1/sd-models",
    );
    return response;
  }
  ...
  public async getHypernetworks(): Promise<HyperNetwork[]> {
    const response = await this.get<HyperNetwork[]>(
  ...
  public async getFaceRestorers(): Promise<FaceRestorer[]> {
    const response = await this.get<FaceRestorer[]>(
  ...
  public async getRealesrganModels(): Promise<RealESRGanModel[]> {
    const response = await this.get<RealESRGanModel[]>(
  ...
  public async getPromptStyles(): Promise<PromptStyle[]> {
    const response = await this.get<PromptStyle[]>(
   ...
   */
  public waitForReady(checkInterval: number = 5.0): Promise<boolean> {
